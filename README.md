...
这是一个轻量级、智能化的 Linux DNS 修改脚本。它可以自动检测系统环境，将系统的 DNS 服务器永久修改为 **Google (8.8.8.8)** 和 **Cloudflare (1.1.1.1)**，并防止 DHCP 自动覆盖。
...
对于传统系统，自动使用 chattr +i 锁定文件，防止重启后被云服务商的 DHCP 还原。

👀 幂等性检测：

脚本运行前会自动检查当前 DNS，如果已经是 8.8.8.8，则自动跳过，避免重复重启服务。

🌍 全平台支持：支持 Debian, Ubuntu, CentOS, Alpine, Fedora, Rocky Linux 等主流发行版。
